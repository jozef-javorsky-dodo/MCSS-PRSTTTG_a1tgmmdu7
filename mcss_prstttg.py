...
class EigenvalueComputer(StateComputer):
    def compute(self, matrix: NDArrayF64, _: NDArrayF64) -> NDArrayF64:
        ...
        mask = np.isclose(ev, 1.0)
        steady = evec[:, mask].real
        steady /= steady.sum()
...
class MarkovChainSimulator:
    ...
    def simulate_random_walks(self) -> NDArrayF64:
        steps = self._rng.choice(
            [-1, 1],
            size=(self.config.num_walks, self.config.num_steps)
        )
        ...
        with ThreadPoolExecutor(max_workers=len(self.config.methods)) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.exception("Steady state computation failed with method " + method)
                    raise SimulationError("Failed to compute steady state using " + method) from err
        return results
    ...
    def monte_carlo_estimate(
        ...
        n = mat.shape[0]
        traj = self._rng.integers(0, n, size=self.config.iterations + 1,
                                   dtype=np.int64)
    ...
    def plot_walks(
        ...
        try:
            ...
            if save_path:
                ...
                logger.info("Saved plot to " + str(fp))
            if show:
                plt.show()
            plt.close(fig)
        except Exception as err:
...
class SimulationRunner:
    ...
    def run_random_walk_simulation(self) -> None:
        try:
            ...
            out_file = "markov_walks_" + uuid.uuid4().hex + ".png"
...
def main() -> None:
    ...
    try:
        ...
        logger.error("Main function failed: " + str(err))
    finally:
        plt.close("all")
